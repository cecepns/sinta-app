...
import { Download, Pencil, QrCode, Send, Trash2 } from 'lucide-react';
...
import { getSubMenusListUrl } from '../utils/appUrl.js';
...
export function SubMenusPage() {
  ...
  const subMenusPageLink = useMemo(() => (mid ? getSubMenusListUrl(mid) : ''), [mid]);
  ...
  const handleDownloadQr = async () => {
    ...
    setQrLoading(true);
    try {
      await downloadUrlAsQrPng(
        subMenusPageLink,
        `sinta-sub-menu-m${menuId}.png`
      );
      toastSuccess('Kode QR diunduh. Pindai untuk membuka halaman sub menu ini.');
    } catch (e) {
  ...
  return (
    <div>
      ...
      {mid > 0 && subMenusPageLink && (
        <div className="mb-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="text-xs font-medium text-slate-500">Tautan halaman sub menu (host mengikuti aplikasi / env)</p>
              <p className="mt-1 break-all text-xs text-slate-800">{subMenusPageLink}</p>
            </div>
            <button
              type="button"
              disabled={qrLoading}
              onClick={handleDownloadQr}
              className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-sinta-200 bg-sinta-50 px-3 py-2 text-sm font-medium text-sinta-800 hover:bg-sinta-100 disabled:opacity-50"
            >
              <Download className="h-4 w-4" />
              {qrLoading ? '…' : 'Unduh kode'}
